...
findEntry entryQuery = head <<< filter (isEntry entryQuery)
  where
  isEntry (ByName nameQuery) entry = nameQuery.firstName == entry.firstName && nameQuery.lastName == entry.lastName
...
containsEntry firstName lastName = isNothing <<< findEntry query
  where
  query = ByName { firstName: firstName, lastName: lastName }

removeDuplicates :: AddressBook -> AddressBook
removeDuplicates = nubBy isDup
  where
  isDup entry1 entry2 = entry1.firstName == entry2.firstName && entry1.lastName == entry2.lastName
